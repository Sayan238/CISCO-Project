...
export default function DiagnosisPage({ cases, selectedCaseId, onSelectCase, diagnosis, onOpenReviewModal, reviewState }) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            ...
            disabled={running}
            className="flex items-center space-x-2 px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold shadow-xs transition-all"
          >
            <RefreshCw className={`w-3.5 h-3.5 ${running ? 'animate-spin' : ''}`} />
            <span>{running ? 'Diagnosing...' : 'Re-Run AI Diagnosis'}</span>
          </button>
        </div>
      </div>
