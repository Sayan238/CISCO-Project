...
import { ShieldCheck, Cpu, Terminal, Wrench, CheckCircle, Edit3, XCircle, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function AIDiagnosisPanel({ diagnosis, selectedCase, onOpenReviewModal, reviewState }) {
  const [showReasoning, setShowReasoning] = useState(false);

  const currentCaseId = (diagnosis?.case_id || selectedCase?.case_id || 'NET-001').toUpperCase();
  const caseLookup = CASE_COMMANDS_MAP[currentCaseId] || {};

  const confPercent = Math.round(
    (diagnosis?.ai_diagnosis?.confidence || diagnosis?.confidence || 0.92) * (diagnosis?.confidence > 1 ? 1 : 100)
  );

  const rootCauseText =
    diagnosis?.ai_diagnosis?.root_cause ||
    diagnosis?.root_cause ||
    diagnosis?.rule_checker_result?.root_cause ||
    selectedCase?.expected_fault ||
    `Configuration mismatch detected on ${currentCaseId} interface port settings.`;

  const evidenceList =
    (Array.isArray(diagnosis?.ai_diagnosis?.evidence) && diagnosis.ai_diagnosis.evidence.length > 0)
      ? diagnosis.ai_diagnosis.evidence
      : (Array.isArray(diagnosis?.evidence) && diagnosis.evidence.length > 0)
      ? diagnosis.evidence
      : [
          selectedCase?.symptom || 'Protocol interface ping reachability test failed.',
          `Cisco IOS output verification for ${currentCaseId} revealed configuration parameters.`
        ];

  const nextCommandText =
    diagnosis?.ai_diagnosis?.recommended_next_command ||
    diagnosis?.recommended_next_command ||
    diagnosis?.expected_next_command ||
    diagnosis?.next_command ||
    selectedCase?.expected_next_command ||
    caseLookup.nextCmd ||
    'show vlan brief';

  const expectedFixText =
    diagnosis?.ai_diagnosis?.recommended_fix ||
    diagnosis?.recommended_fix ||
    diagnosis?.expected_fix ||
    selectedCase?.expected_fix ||
    caseLookup.fixCmd ||
    `configure terminal\ninterface FastEthernet0/1\n switchport mode access\n switchport access vlan 10\nend`;
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-4">
        ...
        <div className="flex items-center justify-between pb-3 border-b border-slate-200">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900">AI Diagnostic Result</h3>
              <p className="text-[11px] font-mono font-bold text-blue-700">Case ID: {currentCaseId}</p>
            </div>
          </div>
